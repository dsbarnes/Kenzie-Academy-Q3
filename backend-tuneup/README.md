...
The timeit interface can be a bit confusing to use from within a program, so here are some tips:
 ...
 - Actually, you can take the minimum first, and then divide by 'times' b/c that's the same things. If every number in the list is going to be divided by the same number, and we want to know the smallest one, it's the same as whaterver is smallest before the division, and then just the once division process.
